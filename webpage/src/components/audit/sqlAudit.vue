            ...
            <Table border :columns="columns" :data="tableData" stripe ref="selection"
              ...
              <template slot-scope="{ row, index }" slot="action">
                <div>
                  ...
                  <Button type="text" @click="gotolink(row)" v-if="row.status !== 2 && row.status !==3" size="small">
                    同步正式环境
                  </Button>
    ...
    methods: {
      ...
      gotolink (row) {
            if (row.type === 0) {
              this.$router.push({
                name: 'orderlist1',
                query: {workid: row.work_id, id: row.id, status: 1, type: row.type, basename: row.basename}
              })
            } else {
              this.$router.push({
                name: 'orderlist2',
