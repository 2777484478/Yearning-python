...
export default {
  ...
  //   this.formItem.connection_name = '阿里云';
  // },
  mounted () {
    ...
    axios.get(`${this.$config.url}/detail?workid=${this.$route.query.workid}&status=${this.$route.query.status}&id=${this.$route.query.id}`)
      .then(res => {
        // this.TableDataNew = res.data.data[1].sql
        ...
        for (let i = 0; i < data3.length; i++) {
          ...
          if (i === 0) {
            ...
            this.formDynamic += res.data.data[i].sql + ';'
          }
        }
        // this.formDynamic = res.data.data[1].sql + ';' + res.data.data[2].sql + ';'
      })
      .catch(error => {
        this.$config.err_notice(this, error)
      })
